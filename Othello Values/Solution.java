...
public class Solution {
	...
	static float minimax_value(int board[][], boolean whiteTurn, int searchDepth, float alpha, float beta) {
		...
		if (whiteTurn) {
			...
			result = maxValue(board, searchDepth, depth, alpha, beta);
		} else {
			result = minValue(board, searchDepth, depth, alpha, beta);
		}
		return (float) result;
	}

	private static float maxValue(int board[][], int searchDepth, int depth, float alpha, float beta) {
		...
		if (checkGameOver(board) == NOBODY) {
			...
			if (legalMoves.size() > 0) {
				for (Move m : legalMoves) {
					...
					float returnVal = minValue(newBoard, searchDepth, depth, alpha, beta);
					if (returnVal > 60) {
						return returnVal;
					}
					v = Math.max(v, returnVal);
					if (v >= beta) {
						return v;
					}
				...
				v = minValue(board, searchDepth, depth, alpha, beta);
			}
		} else {
	...
	private static float minValue(int board[][], int searchDepth, int depth, float alpha, float beta) {
		...
		if (checkGameOver(board) == NOBODY) {
			...
			ArrayList<Move> legalMoves = generateLegalMoves(board, false);
			if (legalMoves.size() > 0) {
				for (Move m : legalMoves) {
					int[][] newBoard = play(board, m, false);
					float returnVal = maxValue(newBoard, searchDepth, depth, alpha, beta);
					if (returnVal > 60) {
						return returnVal;
					}

					v = Math.min(v, returnVal);
					if (v <= alpha) {
						return v;
					}
				...
				v = maxValue(board, searchDepth, depth, alpha, beta);
			}
		} else {
