...
public class OthelloSolver0 {
	...
	static float minimax_value(int board[][], boolean whiteTurn, int searchDepth, float alpha, float beta) {
		...
		int depth = -1;
		resursion(board, whiteTurn, searchDepth, depth, result, alpha, beta);
		
		
		return (float) result[0];
	}

	private static float resursion(int board[][], boolean whiteTurn, int searchDepth, int depth, float[] result, float alpha, float beta) {
		if (checkGameOver(board) == NOBODY) {
			ArrayList<Move> legalMoves = generateLegalMoves(board, whiteTurn);

			depth++;
			if (depth == searchDepth) {
				return evaluationFunction(board, whiteTurn);
			}
			if (legalMoves.size() > 0) {
				...
				if (whiteTurn) {
					...
					for (Move m : legalMoves) {
						...
						float distance = resursion(newBoard, !whiteTurn, searchDepth, depth, result, alpha, beta);
